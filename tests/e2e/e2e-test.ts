...
async function testCodexDelegation() {
  ...
  const tool = createCodexExecTool();

  // 1. 空任务（不需要网络）
  const r1 = await tool.execute({ task: "" }, { workspaceRoot: WORKSPACE, sessionId: "test", yolo: true, env: {} });
  assert("空任务返回错误", !r1.ok, r1.summary.slice(0, 100));

  // 2. 测试 Codex 是否可用（快速 ping）
  let codexAvailable = false;
  try {
    const ping = await tool.execute(
      { task: "echo 'ping'", timeout: 15 },
      { workspaceRoot: WORKSPACE, sessionId: "test", yolo: true, env: {} },
    );
    codexAvailable = ping.ok;
  } catch {
    codexAvailable = false;
  }

  if (!codexAvailable) {
    console.log("  ⏭️ Codex API 不可达（跳过网络测试）");
    console.log("  ⏭️ 注意：kata-agent 已正确调用 codex CLI，但 OpenAI API 当前不可访问");
    return;
  }

  // 3. 简单命令
  console.log("  发送简单委派任务...");
  const r2 = await tool.execute(
    { task: "echo 'kata-agent-e2e-ok'", timeout: 60 },
    { workspaceRoot: WORKSPACE, sessionId: "test", yolo: true, env: {} },
  );
  assert("Codex 简单命令执行", r2.ok, r2.summary.slice(0, 200));
  assert("输出包含预期内容", r2.summary.includes("kata-agent-e2e-ok"), r2.summary.slice(0, 200));

  // 4. 文件读取委派
  console.log("  发送文件操作委派...");
  const r3 = await tool.execute(
    { task: "cat package.json | head -5", timeout: 60 },
    { workspaceRoot: WORKSPACE, sessionId: "test", yolo: true, env: {} },
  );
  assert("Codex 文件读取", r3.ok, r3.summary.slice(0, 200));
  assert("输出包含 package.json 内容", r3.summary.includes("name") || r3.summary.includes("kata"), r3.summary.slice(0, 200));

  console.log("  Codex 委派测试完成");
}
...
async function main() {
  ...
  // 测试 2: Codex 委派（测试委派机制本身）
  await testCodexDelegation();
